...
import { Plus, Upload, Trash2, Edit, Package, Users, ShoppingCart, DollarSign, Eye, EyeOff, Clock, BarChart3, X, Truck, CheckCircle } from "lucide-react";
      ...
      <div className="container mx-auto px-6 py-12">
        <header className="flex justify-between items-center mb-12 border-b border-white/5 pb-8">
          <div>
            ...
            <p className="text-zinc-500 text-sm">Manage your repository and collectors</p>
          </div>
        ...
        <div className="flex gap-8 mb-12 overflow-x-auto pb-4 border-b border-white/5 no-scrollbar">
          ...
          >
            <Plus size={14} className={activeTab === 'products' ? 'text-white' : 'text-zinc-700'} /> Add Product
          </button>
          <button 
            onClick={() => setActiveTab('products')}
            className={`flex items-center gap-2 text-[11px] uppercase tracking-[0.2em] font-bold transition-all whitespace-nowrap ${activeTab === 'products' ? 'text-white' : 'text-zinc-500 hover:text-zinc-300'}`}
          >
            <Package size={14} className={activeTab === 'products' ? 'text-white' : 'text-zinc-700'} /> View Products
          </button>
          <button 
            onClick={() => setActiveTab('orders')}
          ...
          >
            <Clock size={14} className={activeTab === 'orders' ? 'text-white' : 'text-zinc-700'} /> Recent Orders
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          <div className="bg-zinc-900/50 p-6 rounded-2xl border border-white/5 flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] uppercase tracking-widest text-zinc-500 font-bold">Total Products</p>
              <h3 className="text-2xl font-serif text-white">{products.length}</h3>
            </div>
          </div>
          <div className="bg-zinc-900/50 p-6 rounded-2xl border border-white/5 flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-white/5 flex items-center justify-center text-white">
            ...
            <div>
              ...
              <h3 className="text-2xl font-serif text-white">{orders.length}</h3>
            </div>
          </div>
        </div>
        ...
        {activeTab === 'products' ? (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-1">
              <div className="bg-zinc-900/50 p-8 rounded-2xl border border-white/5 sticky top-32">
                <h2 className="text-xl font-serif text-white mb-8">{isEditing ? "Edit Product" : "Add New Product"}</h2>
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div>
                    <label className="text-[10px] uppercase tracking-widest text-zinc-500 font-bold block mb-2">Title</label>
                    <input 
                      type="text" required
                  ...
                  <div>
                    <label className="text-[10px] uppercase tracking-widest text-zinc-500 font-bold block mb-2">Price ($)</label>
                    <input 
                  ...
                  <div>
                    <label className="text-[10px] uppercase tracking-widest text-zinc-500 font-bold block mb-2">Product Images</label>
                    <div className="space-y-4">
                      {/* Uploaded Images */}
                      {formData.images.length > 0 && (
                        <div className="flex flex-wrap gap-2">
                          {formData.images.map((url, index) => (
                            <div key={index} className="w-20 h-20 rounded-lg overflow-hidden border border-white/10">
                              <ImageWithFallback
                                src={getImageUrl(url)}
                      ...
                      {selectedFiles.length > 0 && (
                        <div className="flex flex-wrap gap-2">
                          {selectedFiles.map((file, index) => (
                            <div key={index} className="relative w-20 h-20 rounded-lg overflow-hidden border border-yellow-400/50">
                              <img src={URL.createObjectURL(file)} className="w-full h-full object-cover" alt={`Preview ${index + 1}`} />
                              <button
                                type="button"
                      ...
                      <label className="flex flex-col items-center justify-center border-2 border-dashed border-white/10 rounded-xl hover:border-yellow-400/50 transition-colors cursor-pointer p-4 group">
                        <Plus size={20} className="text-zinc-500 group-hover:text-yellow-400 mb-1" />
                  ...
                  <div>
                    <label className="text-[10px] uppercase tracking-widest text-zinc-500 font-bold block mb-2">Description</label>
                    <textarea 
                      required rows={4}
                      className="w-full bg-black border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-white/30"
                  ...
                  <div className="flex gap-4">
                    ...
                    >
                      {isSubmitting ? (
                        ...
                        isEditing ? "Update Piece" : "Archive Piece"
                      )}
                    </button>
                    {isEditing && (
                      <button type="button" onClick={resetForm} className="px-4 py-3 bg-zinc-800 rounded-xl hover:bg-zinc-700">
                        <X size={20} />
                      </button>
                    )}
                  </div>
            ...
            <div className="lg:col-span-2">
              <div className="space-y-4">
                {products.map(product => (
                  <div key={product.id} className="bg-zinc-900/30 p-6 rounded-2xl border border-white/5 flex items-center justify-between hover:bg-white/[0.02] transition-colors">
                    <div className="flex items-center gap-6">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-zinc-500 uppercase tracking-widest">${product.price}</p>
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => handleEdit(product)} className="p-3 bg-white/5 rounded-xl hover:bg-white/10 text-white transition-all">
                        <Edit size={16} />
                      </button>
                      <button onClick={() => handleDelete(product.id)} className="p-3 bg-white/5 rounded-xl hover:bg-red-500/10 text-red-500 transition-all">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </div>
          ...
          <div className="space-y-6">
            {orders.map(order => (
              <div key={order.id} className="bg-zinc-900/30 p-8 rounded-3xl border border-white/5 flex flex-col md:flex-row md:items-center justify-between gap-8">
                <div className="flex gap-6">
                  <div className="w-12 h-12 rounded-full bg-white/5 flex items-center justify-center text-zinc-500">
                    <Package size={24} />
                  </div>
                  <div>
                    <h3 className="text-white font-serif text-xl"><span className="text-zinc-500 text-[10px] uppercase tracking-widest mr-2">Name:</span>{order.customerName}</h3>
                    <p className="text-sm text-zinc-500"><span className="text-zinc-500 text-[10px] uppercase tracking-widest mr-2">Email:</span>{order.customerEmail} • <span className="text-zinc-500 text-[10px] uppercase tracking-widest mr-2">ID:</span>Piece #{order.productId}</p>
                    <p className="text-[10px] uppercase tracking-widest text-zinc-700 mt-2"><span className="text-zinc-500 text-[10px] uppercase tracking-widest mr-2">Address:</span>{order.shippingAddress}</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <div className={`px-4 py-1.5 rounded-full text-[10px] uppercase tracking-widest font-bold border ${
              ...
              <div className="ml-2">
                {formData.images.length > 0 ? (
                  formData.images.map((img, i) => (
                    <div key={i} className="truncate">[{i}]: {img}</div>
                  ))
                ) : (
                  <div>No images</div>
                )}
