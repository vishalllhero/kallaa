...
export default function ProductDetail() {
  ...
  // Debug logging
  console.log('[ProductDetail] Component render - URL ID:', id, 'Product ID:', product?.id, 'Loading:', loading);
  ...
  const fetchProduct = useCallback(async (productId: string) => {
    ...
    try {
      ...
      console.log('[ProductDetail] Setting product state with:', data);
  ...
  useEffect(() => {
    ...
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      e.preventDefault();
      e.returnValue = 'This exclusive piece may not be available again. Are you sure you want to leave?';
  ...
  if (loading) return <div className="min-h-screen bg-black flex items-center justify-center"><div className="w-12 h-12 border-t-2 border-yellow-400 rounded-full animate-spin" /></div>;
  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-6 pt-24 pb-20 relative z-10">
        ...
        >
          <Link href="/products" className="inline-flex items-center gap-2 text-zinc-500 hover:text-white transition-all duration-300 uppercase text-[10px] tracking-[0.3em] font-bold mb-16 group">
      ...
      {import.meta.env.DEV && (
        <div className="fixed bottom-4 right-4 bg-black/90 text-white p-4 rounded-lg max-w-sm z-50 text-xs">
          ...
          <div>First Image: {product?.images?.[0]?.substring(0, 30) || 'none'}...</div>
        </div>
      )}
      ...
      {isOrderModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          <div className="absolute inset-0 bg-black/90 backdrop-blur-xl" onClick={() => setIsOrderModalOpen(false)} />
          <div className="bg-zinc-900 border border-white/10 rounded-3xl w-full max-w-xl p-12 relative animate-fade-in">
            ...
            <p className="text-zinc-400 mb-12">Please provide your details to initiate the private acquisition process for <span className="text-white">"{product.name}"</span>.</p>

            <form onSubmit={handleOrder} className="space-y-6">
              <div>
                <label className="text-[10px] uppercase tracking-[0.3em] font-bold text-zinc-500 block mb-3">Full Legal Name</label>
                <input 
                  type="text" 
                  required
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label className="text-[10px] uppercase tracking-[0.3em] font-bold text-zinc-500 block mb-3">Professional Email</label>
                  <input 
                    type="email" 
                    required
                ...
                <div>
                  <label className="text-[10px] uppercase tracking-[0.3em] font-bold text-zinc-500 block mb-3">Shipping Region</label>
                  <input 
                    type="text" 
                    required
