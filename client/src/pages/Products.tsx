      ...
      <div className="container mx-auto">
        ...
        <AnimatePresence mode='wait'>
          ...
          >
            <div className="max-w-2xl">
              <h2 className="text-[10px] uppercase tracking-[0.5em] text-yellow-400 font-bold mb-4">Repository</h2>
              <h1 className="text-5xl md:text-7xl font-serif text-white mb-6">The <span className="italic text-zinc-500">Archive</span></h1>
              <p className="text-zinc-400 text-lg leading-relaxed">
                Every digital piece in this archive is unique. These are not editions; 
                they are individual stories painted onto the digital canvas.
              </p>
            </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <h3 className="text-2xl font-serif text-zinc-400 mb-4">The archive is empty</h3>
            <p className="text-zinc-600">New pieces will be added soon. Check back later.</p>
          </div>
        ) : (
          <motion.div
            layout
                ...
                <Link href={`/product/${product.id}`} className="group">
                  <div className="relative aspect-[4/5] overflow-hidden rounded-2xl mb-6 bg-zinc-900 border border-white/5 group-hover:border-white/20 transition-all duration-700">
                    ...
                    <div className="absolute bottom-8 left-8 right-8 flex justify-between items-end transform translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
                      <span className="text-white text-2xl font-serif">${parseFloat(product.price).toLocaleString()}</span>
                  ...
                  <div className="px-2">
                    <h3 className="text-zinc-400 text-xl font-serif mb-1 group-hover:text-white transition-colors">{product.name}</h3>
                    <p className="text-zinc-600 text-[10px] uppercase tracking-[0.3em] font-bold">{product.isSold ? `Curated by ${product.ownerName || 'Unknown'}` : 'Artist Edition'}</p>
                  </div>
                </Link>
              </motion.div>
            ))}
        ...
        <DebugImage images={products.slice(0, 3).map(p => p.images?.[0]).filter(Boolean)} />
      </div>
    </div>
  );
}