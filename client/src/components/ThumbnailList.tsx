...
export default function ThumbnailList({
  ...
  return (
    <div className="flex xl:flex-col gap-4">
      {safeMap(allImages)(Array.isArray(data) ? data : []).map(...)
        (Array.isArray(orders) ? orders : []).map(...)
        (Array.isArray(items) ? items : []).map(...)(image, index) => (
      <motion.button
        key={`thumb-${index}`}
        onClick={() => onSelectImage(index)}
        className={`relative w-20 h-28 rounded-2xl overflow-hidden border-2 transition-all duration-300 ${selectedIndex === index
